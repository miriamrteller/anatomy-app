...
const EXPECTED_STRUCTURES: Record<
  string,
  { name: string; latinName: string; description: string; svgPathId: string }
> = {
  Frontal: {
    name: 'Frontal Bone',
    latinName: 'Os frontale',
    description:
      'Forms the anterior and superior part of the cranium. Contains the frontal sinuses and has a prominent brow ridge.',
    svgPathId: 'Cranium',
  },
  Parietal: {
    name: 'Parietal Bone',
    ...
    description:
      'Forms the sides and roof of the cranium. Two bones (left and right) that meet at the sagittal suture.',
    svgPathId: 'Cranium',
  },
  Temporal: {
    name: 'Temporal Bone',
    ...
    description:
      'Forms the sides and base of the cranium. Contains the ear canal and articulates with the mandible.',
    svgPathId: 'Cranium',
  },
  Occipital: {
    name: 'Occipital Bone',
    ...
    description:
      'Forms the posterior and inferior part of the cranium. Contains the foramen magnum through which the spinal cord passes.',
    svgPathId: 'Cranium',
  },
  Ethmoid: {
    name: 'Ethmoid Bone',
    ...
    description:
      'Forms part of the cranial floor and nasal septum. Contains ethmoidal air cells and contributes to the nasal cavity.',
    svgPathId: 'Cranium',
  },
  Sphenoid: {
    name: 'Sphenoid Bone',
    ...
    description:
      'Located at the base of the cranium, forms the sella turcica which houses the pituitary gland. Has multiple foramina for nerves and vessels.',
    svgPathId: 'Cranium',
  },
  Nasal: {
    name: 'Nasal Bone',
  ...
  Lacrimal: {
    ...
    description:
      'Small bone forming part of the medial wall of the orbit. Contains the lacrimal groove for tear ducts.',
    svgPathId: 'Cranium',
  },
  Zygomatic: {
    name: 'Zygomatic Bone',
    ...
    description:
      'Forms the prominence of the cheek. Articulates with the frontal, temporal, and maxilla.',
    svgPathId: 'Cranium',
  },
  Maxilla: {
    name: 'Maxilla',
    latinName: 'Maxilla',
    description:
      'Forms the upper jaw and palate. Holds the upper teeth and has large sinuses (maxillary sinuses).',
    svgPathId: 'Cranium',
  },
  Vomer: {
    name: 'Vomer',
    latinName: 'Vomer',
    description:
      'Unpaired bone that forms the inferior and posterior part of the nasal septum.',
    svgPathId: 'Cranium',
  },
  Mandible: {
    name: 'Mandible',
    latinName: 'Mandibula',
    description:
      'Forms the lower jaw. Articulates with both temporal bones at the temporomandibular joint.',
    svgPathId: 'Cranium',
  },
  Palatine: {
    name: 'Palatine Bone',
    ...
    description:
      'Forms the posterior part of the hard palate and contributes to the nasal cavity.',
    svgPathId: 'Cranium',
  },
  'Inferior Nasal Concha': {
    ...
    description:
      'Bony ridge in the nasal cavity that projects from the lateral wall. Extends from the maxilla.',
    svgPathId: 'Cranium',
  },
  // Vertebrae samples
  Vertebra_C1: {
    ...
    description:
      'Supports the skull. Lacks a body; consists of anterior and posterior arches with lateral masses.',
  ...
  Vertebra_L5: {
    ...
    description:
      'Fifth lumbar vertebra. Articulates with the sacrum below.',
  ...
  Sacrum: {
    ...
    latinName: 'Os sacrum',
    description:
      'Large triangular bone formed by fusion of 5 sacral vertebrae. Base of the spine, articulates with pelvis.',
    svgPathId: 'Sacrum',
  },
  Coccyx: {
    name: 'Coccyx',
    ...
    description:
      'Small triangular bone formed by fusion of 3-5 coccygeal vertebrae at the base of spine.',
    svgPathId: 'Coccyx',
  },
  Sternum: {
    name: 'Sternum',
    latinName: 'Sternum',
    description:
      'Flat bone in center of chest connecting ribs and clavicles. Protects heart and lungs.',
    svgPathId: 'Sternum',
  },
  Manubrium: {
    name: 'Manubrium',
    ...
    description:
      'Upper portion of the sternum. Articulates with the clavicles and first ribs.',
    svgPathId: 'Manubrium',
  },
};
  ...
  try {
    ...
    for (const struct of structures) {
      ...
      if (!hasSvgPathIdInMetadata) {
        ...
        issues.push(
          `  ⚠️  ${struct.name}: Missing svgPathId in metadata`
        );
      } else {
        passedChecks++;
      }
    ...
    console.log(
      `  ✅ SVG Path IDs: ${structures.length - missingSvgPathIds}/${structures.length}`
    );
    ...
    console.log(
      `  ✅ Metadata svgPathId: ${structures.length - missingSvgPathIdInMetadata}/${structures.length}`
    );
    ...
    for (const [, expected] of Object.entries(EXPECTED_STRUCTURES)) {
      ...
      if (dbStruct.description === expected.description) {
        ...
        issues.push(
          `  ❌ ${expected.name}: Description does not match source data`
        );
      }
    }

    console.log(
      `  ✅ Matched: ${matchedDescriptions}/${Object.keys(EXPECTED_STRUCTURES).length}`
    );
    ...
    for (const struct of structures) {
      for (const svgId of struct.svgPathIds || []) {
        ...
        if (validSvgIds.has(svgId)) {
          ...
          failedChecks++;
          issues.push(
            `  ❌ ${struct.name}: Invalid SVG path ID "${svgId}"`
          );
        }
      }
    }
    ...
    console.log(
      `  ✅ Valid SVG IDs: ${structures.length - invalidSvgIds}/${structures.length}`
    );
    if (invalidSvgIds > 0) {
    ...
    for (const struct of samples) {
      ...
      console.log(
        `     Description: "${struct.description.substring(0, 50)}..."`
      );
      console.log(`     SVG Path IDs: ${struct.svgPathIds?.join(', ') || 'N/A'}`);
      console.log(
        `     Metadata.svgPathId: ${struct.metadata?.svgPathId || '❌ Missing'}`
      );
    }
